...
export class SchwaFormatter extends Formatter {
	constructor(logger: Logger) {
		...
		this.register(AstType.VariableDef, (n) => {
			...
			out += n.token.value
			if (l) out += ' ' + this.printNode(l)
			if (r && r.type == AstType.Literal) out += '[' + this.printNode(r) + ']'
			if (l && l.children.length > 0) out += ' as ' + this.printNode(l.children[0])
			return out
		})
		this.register(AstType.VariableImport, (n) => {
			let l = n.children[0]
			let r = n.children[1]
			let out = ''
		...
		this.register(AstType.Import, (n) => {
			let l = n.children[0]
			let r = n.children[1]
			let out = ''
			if (l && r) out += 'from ' + this.printNode(l) + ' import'
			else if (l) out += 'import ' + this.printNode(l)
			else out += 'import'
			if (r && r.type != AstType.Imports) out += ' '
			if (r) out += this.printNode(r)
			return out
		})
		this.register(AstType.Imports, (n) => {
			let out = '\n'
			for (let i = 0; i < n.children.length; i++) {
				let c = n.children[i]
				if (!c) continue
	...
	protected getDepth(node: AstNode): number {
		...
		if (node.parent.type == AstType.Block ||
			node.parent.type == AstType.Fields ||
			node.parent.type == AstType.Imports) return this.getDepth(node.parent) + 1
