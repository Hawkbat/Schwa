...
class Compiler {
    ...
    debugOutput(mod) {
        if (this.debug) {
            ...
            if (mod.result.ast && mod.result.ast.scope && mod.result.ast.scope.parent)
                console.log(mod.result.ast.scope.parent.toString().replace(/\t/g, '\ \ \ \ '));
...
class Module {
    constructor(name, dir, lines) {
        ...
        if (lines)
            ...
            this.lines = fs.readFileSync(dir, 'utf8').split(/\r?\n/g);
